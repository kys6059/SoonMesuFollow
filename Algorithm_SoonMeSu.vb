...
Module Algorithm_SoonMeSu
    ...
    Public O_스토케스틱_과매수_기준 As Single = 80  '이건 O에만 적용함, N은 반대로 안좋아서 적용하지 않음
    ...
    '20250523    O_CNT_002_A_33_B_8_C_5_D_1_E_95000_F_123000_G_20_H_20_I_0.5_J_0.5_K_30_L_1.4_M_140000    24년10월1일 이후만 따로 해서 나온 결과임
    ...
    Public Sub CalcAlgorithm_Q(ByVal 일분옵션데이터_CurrentIndex As Integer)
        ...
        If Val(순매수리스트(currentIndex_순매수).sTime) >= startTime And Val(순매수리스트(currentIndex_순매수).sTime) <= endTime Then
            ...
            Dim 선물순매수 As Long = 최종순매수값가져오기(3)
            Dim 외국인현물순매수 As Long = 최종순매수값가져오기(1)

            Dim 선물순매수_절대치 As Long = Math.Abs(선물순매수)
            Dim 외국인현물순매수_절대치 As Long = Math.Abs(외국인현물순매수)

            If 선물순매수 * 외국인현물순매수 <= 0 Then Return '곱해서 음수이면 빠진다
            ...
            If 선물순매수 > 0 Then  '  콜 방향
                ...
                If 선물순매수_절대치 > Q_선물발생기준 * offset And 외국인현물순매수_절대치 > Q_외국인현물발생기준 * offset Then  '선물, 현물 둘다 매도나 매수중이면
    ...
    'U 알고리즘 : 시작하지마자 약 5분 정도에 외국인 선물 + 현물이 같은 방향으로 미친듯이 나갈 때 해당 방향 옵션을 매우 작은 양을 매수하고 하루종일 버티는 알고리즘 - 즉 베팅하는 것임. 마지막날만 유효할 듯
    Public U_선물발생기준 As Long = 1000
    Public U_외국인현물발생기준 As Long = 1000
    Public U_시작시간 As Integer = 90500
    Public U_마감시간 As Integer = 90800
    Public U_매도시간 As Integer = 151500

    'Public Q_tick_count_기준 As Integer = 20   'S 알고리즘에서 사용 중임
    'Public O1_짧을때_보정치 As Single = 0.8    'O 알고리즘에서 사용중임

    Public Sub CalcAlgorithm_U(ByVal 일분옵션데이터_CurrentIndex As Integer)

        Dim startTime As Integer = U_시작시간
        Dim endTime As Integer = U_마감시간
        ...
        If Val(순매수리스트(currentIndex_순매수).sTime) >= startTime And Val(순매수리스트(currentIndex_순매수).sTime) <= endTime Then
            ...
            If 선물순매수 > 0 Then  '  콜 방향
                ...
                If 선물순매수_절대치 > U_선물발생기준 * offset And 외국인현물순매수_절대치 > U_외국인현물발생기준 * offset Then  '선물, 현물 둘다 매도나 매수중이면

                    If Get상관계수상태() Then

                        Dim str As String = String.Format("U 신호 발생 콜풋 : {0} 방향", 0)
                        Dim shinho As 순매수신호_탬플릿 = MakeSoonMesuShinho("U", 0)
            ...
            Else ' 풋 방향
                ...
                If 선물순매수_절대치 > U_선물발생기준 * offset And 외국인현물순매수_절대치 > U_외국인현물발생기준 * offset Then  '선물, 현물 둘다 매도나 매수중이면
                    ...
                    If Get상관계수상태() Then
                        ...
                        Dim str As String = String.Format("U 신호 발생 콜풋 : {0} 방향", 1)
                        Dim shinho As 순매수신호_탬플릿 = MakeSoonMesuShinho("U", 1)
                        SoonMesuShinhoList.Add(shinho)
    ...
    Private Function is동일방향신호가살아있나(ByVal callput As Integer) As Boolean
        ...
        If SoonMesuShinhoList IsNot Nothing Then
            For i As Integer = SoonMesuShinhoList.Count - 1 To 0 Step -1
                ...
                ElseIf s.A08_콜풋 <> callput Then

                    Return False '방향이 다른게 있었다면 false를 리턴
    ...
    Private Function 살아있는신호확인하기_Q(ByRef s As 순매수신호_탬플릿, ByVal 일분옵션데이터_CurrentIndex As Integer) As String
    ...
    Public Function O_선물상관계수_계산하기(ByVal index As Integer, ByVal t As Integer) As Single

        Dim 상관계수계산인덱스길이_보정 As Integer = 상관계수계산인덱스길이

        If 상관계수계산인덱스길이 > index Then
            상관계수계산인덱스길이_보정 = Math.Round(상관계수계산인덱스길이 * O1_짧을때_보정치, 0)
        End If

        Dim startPoint As Integer = Math.Max(0, index - 상관계수계산인덱스길이_보정)

        Dim ret As Single = Correl(t, startPoint, index - 1)
    ...
    Public Sub CalcAlgorithm_S(ByVal 일분옵션데이터_CurrentIndex As Integer)
        ...
        If Val(순매수리스트(currentIndex_순매수).sTime) >= startTime And Val(순매수리스트(currentIndex_순매수).sTime) <= endTime Then
            ...
            If SoonMesuShinhoList IsNot Nothing Then
                If SoonMesuShinhoList.Count > 0 Then Return
            End If

            If 순매수리스트(currentIndex_순매수 - 1).상관계수(1) < S_상관계수최저기준 Or 순매수리스트(currentIndex_순매수 - 1).상관계수(3) < S_상관계수최저기준 Then
                Return
            End If
    ...
    Private Function 살아있는신호확인하기_S(ByRef s As 순매수신호_탬플릿, ByVal 일분옵션데이터_CurrentIndex As Integer) As String
        ...
        '    If 순매수리스트(currentIndex_순매수 - 1).상관계수(1) < S_상관계수해제하향돌파기준 Or 순매수리스트(currentIndex_순매수 - 1).상관계수(3) < S_상관계수해제하향돌파기준 Then
        '        s.A14_현재가격 = 일분옵션데이터(s.A08_콜풋).price(일분옵션데이터_CurrentIndex, 3)
        '        s.A16_이익률 = Math.Round((s.A14_현재가격 - s.A10_신호발생가격) / s.A10_신호발생가격, 3)
        '        s.A21_환산이익율 = Math.Round(s.A16_이익률 - 슬리피지, 3)
        '        '매도사유 = "weak_S_2"
