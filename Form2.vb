...
Public Class Form2
    ...
    Private Sub btn_F2_전체조건반복_Click(sender As Object, e As EventArgs) Handles btn_F2_전체조건반복.Click
        ...
        Dim 손절차() As String = {"09", "08"} 'f
        Dim 익절차() As String = {"17", "16", "15"} 'g
        Dim 매수마감시간후기울기() As String = {"10.0", "08.0", "06.0", "05.0"}
                            ...
                            For f As Integer = 0 To 손절차.Length - 1
                                For g As Integer = 0 To 익절차.Length - 1
                                    For h As Integer = 0 To 매수마감시간후기울기.Length - 1
                                        ...
                                        SoonMesuSimulation_조건 = String.Format("CNT_{0}_A_{1}_B_{2}_C_{3}_D_{4}_E_{5}_F_{6}_G_{7}_H_{8}", cntstr, 선행포인트수마진(a), 순매수판정기준(b), 최대포인트수(c), 상승하락기울기기준(d), PIP_CALC_MAX_INDEX(ee), 손절차(f), 익절차(g), 매수마감시간후기울기(h))
