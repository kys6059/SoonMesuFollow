...
Public Class Form2
    ...
    Private Sub Form2_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        '폼의 켑션
        Text = txt_programversion.Text & "__알고리즘_매수_20260531"
        ...
        'strdt = "241001"  'O 알고리즘의 결과가 확 바뀌는 날
    ...
    '20260219 178일을 베이스로 시험하기 위한 기초  Data B260219_E004 --- 매도조건은 이 조건에서 Sell_CNT_004_L_0.008_M_-0.5_N_0.7_O_1.5_P_0.5_Q_151200
    ...
    Private Sub fullTest_E()
        ...
        Dim E_신호발생기준기울기_temp() As Single = {25, 20, 30}    'B
        Dim E_신호해제기준기울기_temp() As Single = {1.0}    'C  - 현재는 해제 기능을 꺼놨음
        Dim PIP_CALC_MAX_INDEX() As String = {"120"}        'D
        Dim 매수시작시간() As String = {"103000"}           'E
        Dim 매수마감시간() As String = {"150000"}           'F
        Dim 신호최소유지시간() As Integer = {4}             'G
        Dim E_DataSource_temp() As Integer = {1, 3, 4}  '외국인 + 기관, 1:외국인, 2: 기관, 3: 외국인선물, 4: 외국인선물+현물  --------- 기관은 켈리지수가 항상 -로 나와서 완전히 제외함
        Dim 장기이평선_temp() As Integer = {60}
        Dim 기관반대순매수_허용크기비율() As Single = {1}
                                                ...
                                                For k As Integer = 0 To 상관계수계산인덱스길이_temp.Length - 1
                                                    For l As Integer = 0 To O_다시발생시적용배율_temp.Length - 1
                                                        For m As Integer = 0 To timeout_temp.Length - 1
                                                            ...
                                                            SoonMesuSimulation_조건 = SoonMesuSimulation_조건 + String.Format("_A_{0}_B_{1}_C_{2}_D_{3}_E_{4}_F_{5}_G_{6}_H_{7}_I_{8}_J_{9}_K_{10}_L_{11}_M_{12}", O_선물발생기준기울기, O_외국인현물발생기준기울기, O_선물해제기준기울기, O_외국인현물해제기준기울기, O_시작시간, O_마감시간, O_tick_count_기준, O_해제tick_count_기준, Math.Round(선물상관계수최저, 2), Math.Round(외국인현물상관계수최저, 2), 상관계수계산인덱스길이, Math.Round(O_다시발생시적용배율, 2), txt_F2_TimeoutTime.Text)
    ...
    Private Sub fullTest_Q()
        ...
        For a As Integer = 0 To Q_선물발생기준_temp.Length - 1
            For b As Integer = 0 To Q_외국인현물발생기준_temp.Length - 1
                For c As Integer = 0 To Q_시작시간_temp.Length - 1
                    For d As Integer = 0 To Q_마감시간_temp.Length - 1



                        Q_선물발생기준 = Q_선물발생기준_temp(a)

                        Q_외국인현물발생기준 = Q_외국인현물발생기준_temp(b)

                        Q_시작시간 = Q_시작시간_temp(c)
                        Q_마감시간 = Q_마감시간_temp(d)
                        ...
                        SoonMesuSimulation_조건 = SoonMesuSimulation_조건 + String.Format("_A_{0}_B_{1}_C_{2}_D_{3}", Q_선물발생기준, Q_외국인현물발생기준, Q_시작시간, Q_마감시간)
